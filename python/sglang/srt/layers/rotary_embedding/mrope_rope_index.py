...
def get_rope_index_ernie45(
    ...
    **kwargs,
    ...
    if input_ids is not None and (
        ...
        mrope_position_deltas = max_position_ids + 1 - s
        return position_ids, mrope_position_deltas
# Note: copied from modeling_keye.py
# https://git.corp.kuaishou.com/recogpt/models/-/blob/master/versions/v0_9_3/128k/modeling_keye.py?ref_type=heads#L2320
    ...
    if input_ids is not None and (
        ...
        for i, input_ids in enumerate(total_input_ids):
            ...
            if image_grid_thw is not None:
                image_nums = image_grid_thw.size(0)
            else:
                image_nums = 0

            if video_grid_thw is not None:
                video_nums = video_grid_thw.size(0)
            else:
                video_nums = 0
            ...
            llm_pos_ids_list: list = []
            st = 0
            remain_images, remain_videos_frames = image_nums, video_nums
            for _ in range(image_nums + video_nums):
                if image_token_id in input_tokens and remain_images > 0:
                    ed_image = input_tokens.index(image_token_id, st)
                else:
